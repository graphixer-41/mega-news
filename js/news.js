...
const displayId = async (id) => {
  ...
  const h = document.getElementById('fff');
...
const categoriesItem = item => {
  ...
  result.forEach(newsCard => {
      ...
      <p class="fw-bold views">&nbsp${newsCard.total_view === null ? "No " : newsCard.total_view} Views</p></div>
      </div>
      <div class="d-flex mt-1"><div>
      <i class="fa-solid  fa-star"></i></div><div>
      <p class="fw-bold rating">&nbsp${newsCard.rating.number} Rating</p></div>
      </div>
      </div>
      </div>
      </div>
